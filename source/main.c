...
//	cvNamedWindow("img_car_after_resize", 1);
	...
	if ((img_after_preprocess = cvLoadImage("img_after_preprocess.bmp", -1)) == NULL) {
	...
	rects = get_location(img_car_after_resize);			/*得到车牌的位置,起初设计阶段是可以有多个预选位置,但是后来发现不用,所以rects其实只有一个位置,但是也是用一个链表装着的*/
